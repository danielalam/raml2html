...
function render(source, config, onSuccess, onError) {
    ...
    raml2obj.parse(source, function(ramlObj) {
        ...
        ramlObj.resources = _.sortBy(ramlObj.resources, 'relativeUri');
        ramlObj.config = config;

        /**
         * Here we assume that the first folder in the include path corresponds to the app name.
         * Would be nice to have a disignated RAML property for this.
         */
        var groupByDisplayName = _.groupBy(ramlObj.resources, function (resource) {
            var name;
            // Use name[1] since [0] is always src
            if (resource.resources && resource.resources[0].file) {
                name = resource.resources[0].file.split('/');
                name = name[1];
            } else if (resource.methods && resource.methods[0].file) {
                name = resource.methods[0].file.split('/');
                name = name[1];
            } else {
                name = resource.file || 'no name';
            }
            return name.toUpperCase();
        });

        // Converting into array form and sorting methods uri's
        ramlObj.resourcesByDisplayName = [];

        // Adding some recursive sorting such that all relative uri are in aplphabetical order
        var sortResources = function (resources) {
            resources = _.sortBy(resources, 'relativeUri');
            _.forEach(resources, function (r){
                if (r.resources) {
                    r.resources = sortResources(r.resources);
                }
            })
            return resources;
        }

        _.forEach(groupByDisplayName, function (g, k) {
            ramlObj.resourcesByDisplayName.push({name: k, resources: sortResources(g)});
        })
        // Sorting by app name
        ramlObj.resourcesByDisplayName = _.sortBy(ramlObj.resourcesByDisplayName, 'name');
